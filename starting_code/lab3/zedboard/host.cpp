...
int main(int argc, char** argv)
{
  ...
  if ( myfile.is_open() ) {
    ...
    for (int i = 0; i < N; ++i) {
      bit64_t input;
      input(inputs[i].length()-1, 0) = inputs[i](inputs[i].length()-1, 0);
      int64_t in = input;
      int nbytes = write(fdw, (void*)&in, sizeof(in));
      assert(nbytes == sizeof(in));
    }
    for (int i = 0; i < N; ++i) {
      int32_t output;
      int output_value;
      int nbytes = read(fdr, (void*)&output, sizeof(output));
      assert(nbytes == sizeof(output));
      output_value = output;
      if(output_value != expecteds[i])
        error++;
    }
    timer.stop();
    num_test_insts = N; 
