...
# collect-result.tcl 
#============================================================================
# @brief: A Tcl script that collect and dump out results from matmult csim & synthesis.
...
if { ! [ file exists "./result/${filename}"] } {
  ...
  if { [ file exist "${hls_prj}/solution1/sim/report/matmult_cosim.rpt" ] } {
...
#---------------------------
set fp [open "${hls_prj}/solution1/syn/report/matmult_csynth.xml" r]
set file_data [read $fp]
close $fp
set data [split $file_data "\n"]
foreach { pattern } {
...
if { [ file exist "${hls_prj}/solution1/sim/report/matmult_cosim.rpt" ] } {
  set fp [open "${hls_prj}/solution1/sim/report/matmult_cosim.rpt" r]
  set file_data [read $fp]
  close $fp
  set data [split $file_data "\n"]
  foreach { pattern } { 
